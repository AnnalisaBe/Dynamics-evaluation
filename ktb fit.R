...
#Title: r = kt^b fitting

#Content: given r (distance of the front of the signal from the starting point at aeach time point) and t (time), estimates b and k. 
#If b is close to 0.5, progression fo the sginal matches diffusion dynamcis.
...
#create a workbook to populate with results
workbook_sqrt <- createWorkbook(paste(series, "_results_ktb", ".xlsx"))
...
model_t <- nls( L ~ k*T^b, start=c(b=0.8, k=20), data = df) #fits the model on your data
cf <- coef(model_t)

coeff_t <- data.frame(replicate='ktb')

coeff_t$b <- cf[1]
coeff_t$k <- cf[2]
datamodel <- summary(model_t)
coeff_t$SEb <- datamodel$coefficients[1,2]
...
plot <- ggplot()+
  ...
  annotate(geom="text", x=150, y=455, label=paste("b=",b),
           size=12)+
...
ggsave(paste(EXP, series, "_ktb", ".png"), plot, width = 8, height = 6, dpi = 450, device = 'png')
...
ggsave(paste(EXP, series, "_ktb", ".pdf"), plot, width = 8, height = 6, dpi = 450)



addWorksheet(workbook_sqrt, 'coefficients_non weight')
writeData(workbook_sqrt, 'coefficients_non weight' , coeff_t)
saveWorkbook(workbook_sqrt, file=paste(series, "_results_ktb", ".xlsx"), overwrite = TRUE)

addWorksheet(workbook_sqrt, 'model_values_non weight')
writeData(workbook_sqrt, 'model_values_non weight' , model_t_df)
saveWorkbook(workbook_sqrt, file=paste(series, "_results_ktb", ".xlsx"), overwrite = TRUE)
